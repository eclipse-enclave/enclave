...
### Distribution assets
- [`assets_embed.go`](../assets_embed.go) embeds the runtime build context, documentation, extensions, and gateway proxy build sources in standalone CLI binaries. Debian and RPM builds select [`assets_noembed.go`](../assets_noembed.go) with the `enclave_no_embed` build tag because package-managed builds install the same tree next to the executable. Package definitions live under [`debian/`](../debian/) and [`packaging/rpm/`](../packaging/rpm/).
...
- **Extension-contributed host commands**: an installed extension's `commands/host/` directory (`model.CommandsDirName`/`model.CommandsHostDirName`, shared with the user's own command tree) is scanned as a third source, after the user's own `host/` and `session/` trees, so a name the user already uses is never taken over by an install. Only the user extension root is read, matching where the installer writes: a built-in extension's own tree is unpacked into a content-addressed cache, which is not a place a host-executed script should come from, and a built-in that wants a verb adds a real command instead. A hand-made directory under the user extension root is read like an installed one (including one overriding a built-in), since it carries the same trust as the user's `commands/` tree. Two extensions claiming one name resolve deterministically (tools before features, alphabetical within a kind) and every loser is warned about. A non-executable file in an extension's tree is ignored silently rather than warned about, unlike one in the user's own tree, because it belongs to the extension and the next update would undo a `chmod`. The directory is host-side alone: `overlayNamedExtensionDirs` skips it, so it reaches neither the build context nor the image identity hash and editing a command forces no rebuild. `TreeHash` still covers it, so a hand-edited command counts as a local modification, and `changesReachImage` keeps an update confined to it from promising a rebuild. `extinstall.hostCommandNames` is the single definition of which files count, shared by the capability summary, `Inventory`, and `remove`. The commands are reported as the capability summary's first row (they are the only capability not bounded by the sandbox), repeated on the diff an update renders, and carried in the `hostCommands` field of both `list --json` and the add/update/remove result envelope. `config.validateCommandsDir` warns about anything under `commands/` other than `host/`, which would otherwise be inert in every direction.
...
- **Network isolation**: by default, a gateway sidecar (dnsmasq + transparent proxy) restricts outbound domains. DNS blocks unknown domains, and the proxy is passthrough-by-default with MITM only for hosts that need secret release rewriting unless `network_log=requests` forces MITM for all allowlisted HTTPS. The session start runs under a context that SIGINT/SIGTERM cancels: a sidecar whose start fails or is interrupted is removed again, and a sidecar orphaned by an earlier interrupted start of the same session name is removed (`backend.StaleGatewayRemover`) before the OAuth port checks run. Once the session is attached, the run is detached from that context; SIGINT/SIGTERM aimed at the enclave process are forwarded to the engine child instead, which proxies them into the container.
...
- **Gateway audit events**: the gateway emits JSONL network events to the host-mounted network log path (`~/.local/state/enclave/projects/<hash>/<tool>/logs/network.log`), all through the shared `netlog.Event` contract. Default `network_log=coarse` records one `tcp` event per TLS connection, written at the ClientHello before the tunnel opens, plus per-request `http` events for plaintext HTTP and for the hosts already MITM'd for secret release; connection reuse therefore hides request volume, and successful DNS lookups are not recorded at all. `network_log=requests` adds request-level HTTP/HTTPS audit events by forcing allowlisted HTTPS through MITM. Two more writers share the file: the DNS audit translator appends `dns` denials, and the host appends a `session` marker at each gateway start. Every event carries the session name, because sessions of the same project and tool share one file. The host rotates the log at session start when it exceeds `netlog.MaxLogBytes` (32 MB), copying it to `network.log.1` and truncating it in place so a session already running keeps writing to the file it bind-mounted. `enclave network log` reads it back; logged HTTP paths never include query strings.
...
- **Managed skills**:
  ...
  - Precedence is built-in extension, user extension, enabled-feature skills, host passthrough, global shared, global tool-specific, project shared, project tool-specific. Same-named skills replace complete directories.
  - Enabled features may ship a `skills/` directory (`extensions/features/<feature>/skills/` and the user extension tree); only features selected for the session contribute. They overlay as trusted extension content and skip the portable-skill validation applied to shared skills.
  - Shared `SKILL.md` files use `skills_validation`: `strict` (default) checks portable frontmatter; `agent` requires a regular, readable file and leaves metadata validation to the agent. Invalid shared skills warn and are skipped. Both modes ignore symlinked skill directories and symlinks inside skills. Tools without `skillsDir` are unaffected.
