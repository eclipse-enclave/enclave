...
func vncViewerCommand(res *Result) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "vnc-viewer [container-or-session-name]",
...
to --name. If it is omitted, the current project's only VNC-enabled container
is used. If several are running, names are listed and one must be passed
explicitly.
