...
- `enclave_apt_get <apt-get args...>`: `apt-get` under `enclave_retry`, reading apt's `APT::Status-Fd` channel to report the overall download percentage on the same interval as the curl heartbeat. `update` runs with `APT::Update::Error-Mode=any`, so an index that could not be fetched fails the command (apt otherwise exits 0 and keeps stale or no lists); `install` downloads first, under the retries and the attempt timeout, and unpacks second without a wall-clock limit, so dpkg is never interrupted. Use it for `update` and `install` in root-phase scripts. Root-phase steps run with the apt cache mounts, so do not `apt-get clean` or remove `/var/lib/apt/lists` in an `install.sh`: that only discards the shared cache.
