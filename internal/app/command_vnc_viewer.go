...
// running session of the current project. A container name, a container ID, or
// a session name passed to --name may be given as the single positional
// argument. Without it the project's only VNC-enabled session is used.
func runVNCViewer(projectDir string, opts model.Options) int {
	...
	session, binding, err := resolveVNCTarget(ctx, be, projectDir, opts)
	if err != nil {
		logx.Errorf("%v", err)
		return 1
	}
...
// resolveVNCTarget picks the session whose display to open and returns its
// published RFB binding.
//
// A named session goes through the shared session resolver, so a session name
// passed to --name addresses it here exactly as it does in attach, stop, and
// theia. The no-argument form selects among the current project's VNC-enabled
// sessions only: a session without the feature has no display to open, so it
// is never the implied target.
func resolveVNCTarget(ctx context.Context, be backend.Backend, projectDir string, opts model.Options) (backend.Session, backend.PortMapping, error) {
	if len(opts.CmdArgs) == 0 {
		project, err := config.ResolveProjectFromDir(projectDir)
		if err != nil {
			return backend.Session{}, backend.PortMapping{}, fmt.Errorf("resolve project: %w", err)
		}
		sessions, err := be.List(ctx, backend.SessionFilter{RunningOnly: true, ProjectHash: project.Hash})
		if err != nil {
			return backend.Session{}, backend.PortMapping{}, fmt.Errorf("list containers: %w", err)
		}
		return autoSelectVNCSession(sessions)
	}
	...
	if err != nil {
		return backend.Session{}, backend.PortMapping{}, err
	}
	binding, ok := vncPortBinding(session)
	if !ok {
		return backend.Session{}, backend.PortMapping{}, fmt.Errorf(
			"%s runs without the vnc feature (restart it with `%s --features +vnc`)", session.Ref.Name, model.AppName)
	}
	return session, binding, nil
}

// autoSelectVNCSession resolves the no-argument form. Sessions without the vnc
// feature have no published RFB binding, which is what distinguishes "no
// session" from "session without a VNC display" in the error messages.
func autoSelectVNCSession(sessions []backend.Session) (backend.Session, backend.PortMapping, error) {
	...
	default:
		...
		return backend.Session{}, backend.PortMapping{}, fmt.Errorf(
			"multiple running enclave containers with a VNC display. Pass a container or session name explicitly:\n  %s", strings.Join(names, "\n  "))
	}
}
