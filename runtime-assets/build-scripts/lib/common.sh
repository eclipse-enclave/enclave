...
# ---------------------------------------------------------------------------
# The network helpers for build-time downloads (enclave_retry, enclave_curl,
# enclave_apt_get, and their settings) live in fetch.sh, which the Dockerfile
# copies into the image ahead of the package installs on its own.
# shellcheck source=runtime-assets/build-scripts/lib/fetch.sh
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/fetch.sh"
