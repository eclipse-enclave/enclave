...
| `ENCLAVE_BUILD_RETRIES` | How many times an image build is rerun after failing on a transient network error (default `1`). Each rerun starts from the layer cache, so only the failed step repeats |
...
| `ENCLAVE_NET_PROGRESS_INTERVAL_SECONDS` | How often a running download (curl, or apt's package fetches) reports bytes received, percentage, rate, and time left (default `30`; `--progress verbose` uses `5`, `quiet` uses `0`; an explicit value wins) |
