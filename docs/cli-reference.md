...
| `enclave version` | Show the binary version, source commit, and commit date |
...
`enclave version` prints one line suitable for bug reports. Builds from modified source append `-dirty` to the commit; unavailable values are reported as `unknown`.
