...
- **Tool selection** ([`internal/app/tool.go`](../internal/app/tool.go)): `--tool` selects the sandbox profile. The default is unset: the first command that starts a session or builds its image asks once which installed agent to use and saves the answer as `"tool"` in the global config. An `update` with explicit tool arguments rebuilds exactly those images and never reads the default tool, so it does not ask. The menu offers every installed agent profile and never the IDE profiles; non-terminal, `--json` and `--yes` invocations, and every other verb, use `claude` without asking, or the single installed agent when there is exactly one.
