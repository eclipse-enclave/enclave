...
| `ENCLAVE_NET_ATTEMPT_TIMEOUT_SECONDS` | Wall-clock limit per attempt for retried install commands such as `go install` and the download phase of `apt-get install`, and the cap on one npm tarball download (default `1800`, `0` disables) |
