...
- **Extension-contributed host commands**: `usercmd.Discover` scans each installed extension's `commands/host/` under the user extension root as a third source, after the user's own `host/` and `session/` trees, so an install never takes a name already in use. `overlayNamedExtensionDirs` excludes `commands/` from the build context and the image identity hash, while `TreeHash` still covers it. `usercmd.ExtensionCommandNames` is the filter the installer, `list`, and `remove` share. See [Host commands](extensions/README.md#host-commands).
