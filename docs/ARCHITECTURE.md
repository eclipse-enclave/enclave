...
- **Isolation backend**: `--backend` selects the session isolation backend. `docker` is the default and supports the full feature set. `podman` reuses the docker backend and CLI wrapper with a podman engine selected (`internal/docker/engine.go`); rootless podman sessions run with `--userns=keep-id` instead of the rootless-Docker nested sandbox, and devcontainer mode remains docker-only. Experimental `qemu` supports foreground slim/no-feature unrestricted sessions in an Alpine microVM bundle. All backends realize persistent stores from the shared host-directory layout (`internal/backend/hoststore`), so auth, tool config, and persisted env are shared between containers and microVMs.
